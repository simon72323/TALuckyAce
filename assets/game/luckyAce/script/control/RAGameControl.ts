...
import { onBeginGameData } from '../enum/mockData';
...
export class RAGameControl extends Component {
    ...
    private hasGameStarted: boolean = false; // 只有第一次onBeginGame顯示true，reset要設回false
    ...
    private playFirstMultiple(): void {
        const deafultLine = 0;
        this.comboView.showMultipleLabel(deafultLine);
    }
    ...
    private playLineWin(): void {
        const payData = this.model.getBeginGameData()['data']['PayTotal'];
        ...
        this.comboView.setLineWinInfo(linedata, payData, freeGameSpinData);
    ...
    private playScatterWin(): void {
        ...
        this.comboView.setScatterWinInfo(linedata, payData, freeGameData, freeGameSpinData);
    }
}
